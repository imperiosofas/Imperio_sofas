  ...
  return (
    <section id="catalogo" className="section-pad overflow-hidden bg-ink">
            ...
            <SwiperSlide key={product.name} className="!h-auto">
              <motion.article whileHover={enhanced ? { y: -6 } : undefined} transition={{ duration: 0.25 }} className="h-full overflow-hidden rounded-[1.5rem] border border-white/9 bg-card shadow-2xl shadow-black/30">
                <div className="relative aspect-[11/10] overflow-hidden bg-card">
                  <img src={product.image} srcSet={product.srcSet} sizes="(max-width: 639px) calc(100vw - 56px), (max-width: 899px) 65vw, (max-width: 1279px) 47vw, 37vw" width="1100" height="1000" loading="lazy" decoding="async" alt={product.alt} style={{ objectPosition: product.objectPosition }} className="h-full w-full object-cover" />
                </div>
      ...
      <div className="shell mt-8 text-center sm:mt-10">
        ...
        <a href={FULL_CATALOG_URL} target="_blank" rel="noopener noreferrer" className="mx-auto mt-6 flex min-h-12 w-full max-w-sm items-center justify-center gap-3 rounded-full bg-gold px-5 py-4 text-sm font-bold text-ink transition-colors hover:bg-gold-light focus-visible:outline-2 focus-visible:outline-offset-4 focus-visible:outline-gold sm:w-fit sm:max-w-none sm:px-8 sm:text-base">
